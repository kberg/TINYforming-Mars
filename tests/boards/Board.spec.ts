...
describe('Board', function() {
  let board : TharsisBoard; let player : Player; let player2 : Player;

  beforeEach(function() {
    board = TharsisBoard.newInstance(false, new Random(0));
    player = TestPlayers.BLUE.newPlayer();
    player2 = TestPlayers.RED.newPlayer();
  });

  it('getSpace', () => {
    expect(board.getSpace('01').spaceType).eq(SpaceType.LAND);
  ...
  it('getOceansOnBoard', function() {
    expect(board.getWaterCubesOnBoard()).eq(0);

    const space1 = board.spaces[1];
    space1.spaceType = SpaceType.WATER;
    space1.tile = {tileType: TileType.WATER};

    expect(board.getWaterCubesOnBoard(true)).eq(1);
    expect(board.getWaterCubesOnBoard(false)).eq(1);

    const space2 = board.spaces[2];
    space2.spaceType = SpaceType.WATER;
    space2.tile = {tileType: TileType.OCEAN_SANCTUARY};

    expect(board.getWaterCubesOnBoard(true)).eq(2);
    expect(board.getWaterCubesOnBoard(false)).eq(1);
  });
