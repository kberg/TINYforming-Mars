...
export class TestingUtils {
  ...
  public static maxOutOceans(player: Player, toValue: number = 0): Array<ISpace> {
    ...
    for (const space of player.game.board.getSpaces(SpaceType.WATER, player)) {
      if (space.tile !== undefined) continue;
      if (player.game.board.getWaterCubesOnBoard() >= toValue) break;
      player.game.addWaterCube(player, space.id);
      oceans.push(space);
    }
    return oceans;
  };
