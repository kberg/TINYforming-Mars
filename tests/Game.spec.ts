...
import {Banker} from '../src/awards/DryHeat';
...
describe('Game', function() {
  ...
  it('correctly calculates victory points', function() {
    ...
    player2.addProduction(Resources.CREDITS, 10);

    // Our testing player will be 2nd Banker in the game
    player.addProduction(Resources.CREDITS, 7);

    // Two other players will share Thermalist award
    award = new Thermalist();
    game.fundAward(player, award);

    player2.heatCubes = 23;
    player3.heatCubes = 23;
  ...
  it('Does not assign player to ocean after placement', function() {
    ...
    const spaceId: SpaceId = game.board.getAvailableSpacesForWaterCubes(player)[0].id;
    game.addWaterCube(player, spaceId);
