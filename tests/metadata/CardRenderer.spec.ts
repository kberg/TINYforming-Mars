...
describe('CardRenderer', function() {
  ...
  describe('oceans', () => {
    it('success', () => {
      const renderer = CardRenderer.builder((b) => b.water(1));
      const item = renderer.rows[0][0] as CardRenderItem;
      expect(item.type).to.equal(CardRenderItemType.WATER);
      expect(item.amount).to.equal(1);
    });
  });
  describe('oxygen', () => {
  ...
  describe('megacredits', () => {
    it('success - amount inside (always)', () => {
      const renderer = CardRenderer.builder((b) => b.credits(45));
      const item = renderer.rows[0][0] as CardRenderItem;
      expect(item.type).to.equal(CardRenderItemType.CREDITS);
      expect(item.amount).to.equal(45);
      expect(item.showDigit).to.be.false;
      expect(item.amountInside).to.be.true;
    });
    it('size - s', () => {
      const renderer = CardRenderer.builder((b) => b.credits(16, CardRenderItemSize.SMALL));
      const item = renderer.rows[0][0] as CardRenderItem;
      expect(item.type).to.equal(CardRenderItemType.CREDITS);
